...
Compares DP (2-D value iteration) vs RL (DQN with uniform state sampling).
Uniform state injection ensures all 231 states get visited, reducing overfitting.
"""
...
SIGMA_BASE = 0.25  # shared volatility for both DP and RL
SEED = 42
...
def main():
    ...
    # DP and RL use the same sigma_base so they solve the same MDP
    ...
    # ── RL (DQN + improvements for better policy match at extremes) ───
    # train_params: terminal_penalty=0.0 to match DP
    train_params = MarketParams(
        spread_options=SPREAD_OPTIONS,
        terminal_penalty=0.0,
        sigma_base=SIGMA_BASE,
        episode_length=750,  # Longer episodes for more steps
    )
    print("\n[2/4] Training DQN (continuous state, uniform+extreme sampling, prioritized replay) …")
    train_env = MarketMakingEnv(
        ...
        random_init=True,  # Episode starts from random (I, price)
        use_continuous_state=True,  # [inv/max_inv, price_dev/scale] for smoother generalization
        seed=SEED,
    )
    ...
    agent = DQNAgent(
        ...
        tau=0.005,
        seed=SEED,
        use_prioritized_replay=True,  # Oversample rare (extreme inv/price) states
        rare_priority=3.0,
    )
    ep_rewards, _ = agent.train(
        train_env,
        n_episodes=2000,  # 2000 × 750 = 1.5M steps (longer training)
        epsilon_start=1.0,
        epsilon_end=0.02,
        epsilon_decay_steps=500_000,
        uniform_state_interval=8,  # More frequent teleports for better coverage
        extreme_state_prob=0.3,  # 30% of teleports target extreme (inv, price) states
        verbose=True,
    )
    ...
    eval_env = MarketMakingEnv(
        ...
        include_price=True, price_scale=PRICE_SCALE,
        discrete_inventory=True, price_grid=price_grid,
        use_continuous_state=True,  # Must match train env
        seed=EVAL_SEED,
    )
